	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			...
			if "spreadsheets/d/" in item["path"]:
				...
				# item["path"] = "plugin://plugin.video.phimhaynhat/?mode=90&page=0&url=" + urllib.quote_plus(item["path"])
			elif any(service in item["path"] for service in ["4share.vn/d/"]):
				item["path"] = "plugin://plugin.video.phimhaynhat/?mode=38&page=0&url=" + \
				...
				item["path"] = "plugin://plugin.video.phimhaynhat/?mode=3&page=0&url=" + \
					urllib.quote_plus(item["path"])
				item["is_playable"] = True
...
def LoginFShare(uname,pword):
	...
	data = '{"app_key" : "L2S7R6ZMagggC5wWkQhX2+aDi467PPuftWUMRFSn", "user_email" : "phuoclt86@gmail.com", "password" : "0905360044"}' % (uname, pword)
